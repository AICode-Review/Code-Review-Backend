...
A **contracts** finding claims impact in a DIFFERENT file (e.g. "this breaks a caller elsewhere"); a **tests** finding sometimes does too (e.g. "no test covers this" or "an existing test elsewhere now asserts the wrong thing"). For either, you may also be given one or both of:
- `## Other files from this PR` — full content of other files touched by this PR, so you can actually check whether a named caller/test does what the finding claims.
- `## Repository index context` — the same best-effort definitions/callers/related-tests list (by name, file, line — not full source) the original pass had when it made this claim.

Treat these the same way you'd treat the diff: real, checkable evidence, not something to distrust by default just because it's "only" an index lookup rather than something you derived yourself. If a named caller's actual call site (or a related test's presence/assertions) is shown in one of these sections and it matches the claim, that supports `"upheld"`; if it contradicts the claim (e.g. the caller was actually updated, a related test does in fact exist, or a test's assertions were already updated), that supports `"refuted"`. Only fall back to `"uncertain"` on a cross-file claim when neither section was provided, or neither one actually covers the specific caller/test/symbol the finding names.
